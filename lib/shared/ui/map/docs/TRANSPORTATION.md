...
**4. `stop_times.route_id` guna nama PENDEK.** Ia `AGL`, `KJL`, `SPL`,
`MRL`, `SAL` - sedangkan `trips.route_id` dan `routes.route_id` guna id
asas `AG`, `KJ`, `PH`, `MR`, `SA`. Menyambung ikut `route_id` merentas fail
itu memulangkan **sifar baris, secara senyap**. Semua sambungan mesti
melalui `trip_id`. `stop_times.txt` juga datang dengan BOM UTF-8; tanpa
membuangnya, kunci lajur pertama jadi `\uFEFFroute_id`.
...
|-------|--------|------------------------|
| `to` | `trips.trip_headsign` | `Sentul Timur` |
| `seq` | `stop_times.stop_sequence` | 12 |
| `first` | `stop_times.departure_time` | `06:18` |
| `last` | diterbitkan - lihat di bawah | `23:43` |
| `freq` | `frequencies.headway_secs` | Isnin-Jumaat `[3, 5]` minit |

`first` disimpan **sekali**, bukan per jenis hari: masa tren pertama sama
merentas MonFri/Sat/Sun dalam feed ini (disemak - 0 daripada 374 entri
berbeza).

`freq` disimpan per laluan+**arah**, bukan per laluan: MRT Putrajaya dan
LRT Shah Alam ada headway berbeza ikut arah.

### `last` ialah ANGGARAN, bukan fakta
